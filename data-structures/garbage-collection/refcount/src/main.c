...
munit_case(RUN, test_array_set, {
  generic_object_t *foo = new_generic_integer(1);

  generic_object_t *array = new_snek_array(1);
  generic_array_set(array, 0, foo);
...
munit_case(SUBMIT, test_array_free, {
  generic_object_t *foo = new_snek_integer(1);
  generic_object_t *bar = new_snek_integer(2);
  generic_object_t *baz = new_snek_integer(3);

  generic_object_t *array = new_snek_array(2);
  generic_array_set(array, 0, foo);
  generic_array_set(array, 1, bar);
  assert_int(foo->refcount, ==, 2, "foo is now referenced by array");
...
munit_case(RUN, test_vector3_refcounting, {
  generic_object_t *foo = new_generic_integer(1);
  generic_object_t *bar = new_generic_integer(2);
  generic_object_t *baz = new_generic_integer(3);

  generic_object_t *vec = new_generic_vector3(foo, bar, baz);
...
munit_case(RUN, test_int_has_refcount, {
  generic_object_t *obj = new_snek_integer(10);
...
munit_case(RUN, test_inc_refcount, {
  generic_object_t *obj = new_snek_float(4.20);
  assert_int(obj->refcount, ==, 1, "Refcount should be 1 on creation");
...
munit_case(RUN, test_dec_refcount, {
  generic_object_t *obj = new_generic_float(4.20);
...
munit_case(RUN, test_allocated_string_is_freed, {
  generic_object_t *obj = new_snek_string("Hello @wagslane!");
