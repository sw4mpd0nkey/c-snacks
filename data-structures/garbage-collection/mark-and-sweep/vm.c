...
void sweep(vm_t *vm) {
  for (int i = 0; i < vm->objects->count; i++) {
    generic_object_t *obj = vm->objects->data[i];
    if (obj->is_marked) {
      obj->is_marked = false;
    } else {
      generic_object_free(obj);
...
void mark(vm_t *vm) {
  for (size_t i = 0; i < vm->frames->count; i++) {
    ...
    for (size_t j = 0; j < frame->references->count; j++) {
      generic_object_t *obj = frame->references->data[j];
      obj->is_marked = true;
    }
  }
}
...
void trace_blacken_object(stack_t *gray_objects, generic_object_t *ref) {
  generic_object_t *obj = ref;
...
void trace_mark_object(stack_t *gray_objects, generic_object_t *obj) {
...
void frame_reference_object(frame_t *frame, generic_object_t *obj) {
...
void vm_free(vm_t *vm) {
  ...
  for (size_t i = 0; i < vm->objects->count; i++) {
    generic_object_free(vm->objects->data[i]);
  }
...
void vm_track_object(vm_t *vm, generic_object_t *obj) {
